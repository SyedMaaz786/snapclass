...
def student_dashboard():
    ...
    with c2:
        ...
        if st.button("Logout", type='secondary', key='loginbackbtn', use_container_width=True, shortcut="control+backspace"):
...
def student_screen():
    ...
    if photo_source:
        ...
        time.sleep(1.2)  # 👈 ADD DELAY HERE (IMPORTANT)
    ...
    if show_registration:
        with st.container(border=True):
            ...
            try:
                audio_data = st.audio_input('Record a short phrase like I am present, My name is john.doe, etc.')
            except Exception:
